    ...
    if (settings.certificate_requires_final_assignment) {
      ...
      const assignmentCheck = await db.query(
        `SELECT a.id, s.score, s.status, a.score_max
         ...
         WHERE a.course_id = $1 AND a.assignment_scope = 'course'
      ...
      if (!finalA.status || finalA.status !== 'graded') {
        return { eligible: false, message: 'Bạn chưa hoàn thành hoặc chưa được chấm điểm bài kiểm tra cuối khóa.' };
      }
